...
BERT_PATH = ROOT.parent / "bert-base-chinese"
DATA_DIR = ROOT / "data" / "peoples_daily"
CKPT_DIR = ROOT / "outputs" / "checkpointss"
    ...
    for i, tag in enumerate(label):
        if tag.startswith("B-"):
            ...
            for j in range(i + 1, len(label)):
                if label[j] == f"I-{entity_type}":
                    end = j
                else:
                    break
            position = [start, end]
            entities.append({"type": entity_type, text[start:end + 1]: position})
    return entities

def write_predictions_to_file(loader, tokenizer, all_golds, all_preds, save_path):
    """写入结果文件，新增 is_correct 字段，True=完全正确，False=预测错误"""
    results = []
    count = 0
    ...
    for batch in loader:
        ...
        for i in range(len(input_ids)):
            ...
            gold = all_golds[count]
            pred = all_preds[count]
            text = "".join(tokens)

            gold_entities = label2entity(text, gold)
            pred_entities = label2entity(text, pred)
            # 整段标签完全一致则为 True
...
def main():
    ...
    # 写入预测结果文件
    pred_save_path = LOG_DIR / f"predictions_{run_tag}_{split_name}.json"
    write_predictions_to_file(loader, tokenizer, all_golds, all_preds, pred_save_path)
