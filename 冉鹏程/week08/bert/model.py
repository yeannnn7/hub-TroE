...
class CrossEncoder(nn.Module):
    ...
    结构：
      BertModel([CLS] s1 [SEP] s2 [SEP]) → 池化 → Dropout → Linear(H, 2) → logits
    ...
    def __init__(self, bert_path, pool="cls", dropout=0.1, num_hidden_layers=None):
        super().__init__()
        assert pool in ("cls", "mean", "max"), f"pool 须为 cls/mean/max，收到: {pool}"
...
def build_crossencoder(bert_path, pool="cls", dropout=0.1, num_hidden_layers=None):
    """构建 CrossEncoder 并打印参数量。"""
    model = CrossEncoder(bert_path, pool=pool, dropout=dropout,
                         num_hidden_layers=num_hidden_layers)
    _print_param_info(model, f"CrossEncoder (pool={pool}, layers={num_hidden_layers or 12})")
    return model
