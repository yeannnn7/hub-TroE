...
def plot_score_dist_with_errors(results, threshold, save_path, model_label, dataset_name, split):
    """
    ...
    ax.set_title(f"[{dataset_name} {split}] {model_label} Positive vs Negative Distribution")
    ...
    ax.set_xlabel("Score")
    ax.set_title(f"[{dataset_name} {split}] {model_label} Correct vs Error Score Distribution")
    ax.legend(fontsize=8)
...
def parse_args():
    ...
    parser.add_argument("--ckpt", default=None, type=str,
                        help="checkpoint 路径；biencoder 未显式传入时按 loss+pool 自动推断")
...
def main():
    ...
    if args.ckpt:
        ...
        ckpt_path = CKPT_DIR / f"biencoder_{args.loss}_{args.pool}_best.pt"
    else:
    ...
    print(f"数据集: {dataset_name} / {data_path.name}  共 {len(raw_rows):,} 条")
    ...
    if args.model_type == "biencoder":
        ...
        model_label = (
            f"BiEncoder(loss={saved_args.get('loss', args.loss)},"
            f"pool={saved_args.get('pool', args.pool)},threshold={threshold:.2f})"
        )
    ...
    # ── 可视化 ────────────────────────────────────────────────────────────
    fig_suffix = f"{dataset_name}_{args.split}_{args.model_type}"
    if args.model_type == "biencoder":
        fig_suffix = (
            f"{dataset_name}_{args.split}_{args.model_type}_"
            f"{saved_args.get('loss', args.loss)}_{saved_args.get('pool', args.pool)}"
        )
    fig_path = FIG_DIR / f"{fig_suffix}_badcase_dist.png"
    plot_score_dist_with_errors(results, threshold, fig_path, model_label, dataset_name, args.split)
