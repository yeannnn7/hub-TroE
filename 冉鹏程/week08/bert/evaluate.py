...
def eval_crossencoder(model, loader, device):
    ...
    返回 dict：
      ...
      labels   : list[int]
    """
    return eval_crossencoder_with_loss(model, loader, device)[0]


@torch.no_grad()
def eval_crossencoder_with_loss(model, loader, device, criterion=None):
    """
    CrossEncoder 评估：可选返回验证集 loss。

    返回 (metrics, val_loss)：
      metrics  : dict，包含 accuracy / f1 / logits / labels
      val_loss : float | None
    """
    model.eval()
    ...
    accuracy = accuracy_score(labels, preds)
    f1       = f1_score(labels, preds, average="weighted", zero_division=0)
    val_loss = (total_loss / total_samples) if criterion is not None and total_samples > 0 else None
...
def main():
    ...
    if args.model_type == "biencoder":
        ...
        metrics, _ = eval_crossencoder_with_loss(model, loader, device)
        print(f"\n{'='*50}")
