...
def dataset_slug(data_dir):
    """Return a filesystem-friendly dataset name inferred from data_dir."""
    name = Path(data_dir).resolve().name.lower()
    return "".join(ch if ch.isalnum() or ch in ("-", "_") else "_" for ch in name)


def checkpoint_path(method, dataset):
    if dataset == "afqmc":
        return CKPT_DIR / method["ckpt"]
    return CKPT_DIR / f"{dataset}_{method['ckpt']}"


def artifact_name(base_name, dataset):
    if dataset == "afqmc":
        return base_name
    stem = Path(base_name).stem
    suffix = Path(base_name).suffix
    return f"{dataset}_{stem}{suffix}"


# ── 加载并评估单个方法 ─────────────────────────────────────────────────────

def load_and_eval(method, tokenizer, device, data_dir, dataset, bert_path, split, batch_size):
    ckpt_path = checkpoint_path(method, dataset)
...
def plot_comparison_bar(results, save_path, dataset, split):
    """准确率 / F1 对比柱状图"""
    ...
    ax.set_ylim(0, 1.0)
    ax.set_ylabel("Score")
    ax.set_title(f"Method Comparison on {dataset.upper()} {split} (BERT)")
    ax.legend()
...
def parse_args():
    ...
    parser.add_argument("--dataset_name", default=None, type=str,
                        help="checkpoint/log/figure 前缀；默认从 data_dir 目录名推断")
...
def main():
    ...
    dataset = args.dataset_name or dataset_slug(args.data_dir)
    print(f"设备: {device}  数据集: {dataset}  评估集: {args.split}")
    ...
    for m in METHODS:
        ...
        metrics = load_and_eval(
            m, tokenizer, device, args.data_dir, dataset,
            args.bert_path, args.split, args.batch_size,
        )
        if metrics is None:
            continue
    ...
    log = [{k: _to_py(v) for k, v in m.items() if k not in SKIP_KEYS}
           for m in all_results]
    log_path = LOG_DIR / artifact_name("method_comparison.json", dataset)
    ...
    plot_comparison_bar(
        all_results,
        FIG_DIR / artifact_name("method_comparison_bar.png", dataset),
        dataset,
        args.split,
    )
    ...
    if bi_with_sim:
        plot_sim_distributions(bi_with_sim, FIG_DIR / artifact_name("biencoder_sim_distributions.png", dataset))
