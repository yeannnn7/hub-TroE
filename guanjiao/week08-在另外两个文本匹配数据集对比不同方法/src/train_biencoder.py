...
def output_stem(args):
    ...
    method = f"biencoder_{args.loss}"
    return method if dataset == "afqmc" else f"{dataset}_{method}"
...
def main():
    ...
    print(f"设备: {device}")
    print(f"数据集: {args.dataset_name or dataset_slug(args.data_dir)}  数据目录: {args.data_dir}")
    ...
    # ── 训练完成，保存日志 ────────────────────────────────────────────────
    log_path = LOG_DIR / f"{stem}_log.json"
...
def parse_args():
    ...
    parser.add_argument("--dataset_name",      default=None, type=str,
                        help="结果文件名前缀；默认从 data_dir 目录名推断")
