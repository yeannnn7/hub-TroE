...
在验证集/测试集上评估 BERT NER 模型，输出 entity-level F1 和非法序列统计。

使用方式：
  python evaluate_peoples_daily.py                        # 评估 BERT+Linear
  python evaluate_peoples_daily.py --use_crf              # 评估 BERT+CRF
  python evaluate_peoples_daily.py --split test           # 在测试集上评估
"""
...
def main():
    ...
    print("\n" + "=" * 70)
    print(f"模型：{'BERT + CRF' if args.use_crf else 'BERT + Linear'}  |  数据集：人民日报  |  评估集：{split_name}")
    print("=" * 70)
...
def parse_args():
    parser = argparse.ArgumentParser(description="评估人民日报 NER 模型")
    parser.add_argument("--use_crf", action="store_true")
    parser.add_argument("--bert_path", type=Path, default=BERT_PATH)
    parser.add_argument("--batch_size", type=int, default=32)
    parser.add_argument("--split", choices=["validation", "test"], default="test",
                        help="人民日报 test 集有标签，可在 test 集上评估")
