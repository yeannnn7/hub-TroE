...
复用 BERT+Linear / BERT+CRF 模型（model.py），适配人民日报数据集（3类实体：PER/ORG/LOC）。

使用方式：
  python train_peoples_daily.py                        # 训练 BERT+Linear
  python train_peoples_daily.py --use_crf              # 训练 BERT+CRF
...
def main():
    ...
    labels, label2id, id2label = build_label_schema(data_dir=DATA_DIR)
    num_labels = len(labels)
    print(f"BIO 标签数：{num_labels}（O + {num_labels - 1} 个实体标签）")
    print(f"实体类型：{', '.join(t for t in sorted(set(l.split('-')[1] for l in labels if '-' in l)))}")
    ...
    print(f"\n开始训练（{'BERT+CRF' if args.use_crf else 'BERT+Linear'}）@ 人民日报 NER ...")
