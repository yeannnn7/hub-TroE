"""
PDF 解析脚本：将原始出行规则 PDF 转换为结构化文本

教学重点（企业级 RAG 的真实挑战）：
  1. 数字 PDF vs 扫描件：处理方式完全不同
  2. 表格提取：规则文档中大量条款表格，直接按文字流提取会乱序
  3. 页眉/页脚噪声：每页都有文件标题、页码，必须去除
...
class ParsedBlock:
    """
    一个解析块 = 出行规则文档里的一段连续内容（文字段落 or 表格）

    保留 page_num 和 section_path 非常重要——
    RAG 答案引用时能告诉用户"来自第3页，第三章/行李规定"
    """
...
NOISE_PATTERNS = [
    re.compile(r"^.{1,40}(守则|规定|办法|规程|目录|规范)\s*$"),  # 页眉：文件标题
...
def main():
    ...
    if not manifest:
        logger.error("没有找到任何 PDF，请将出行规则 PDF 放入 data/raw_pdf/ 目录")
        return
    ...
    for item in manifest:
        ...
        parser = TravelRuleParser(pdf_path, meta=item)
        parser.parse()
        parser.save()
