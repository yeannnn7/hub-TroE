...
输出格式说明：
  每个 chunk 是一个 dict，包含：
    ...
    - metadata      元信息（供过滤/溯源）
      - category    出行类别（如公交、地铁、铁路、民航、网约车、城市交通）
      - year        年份
      - page_num    来源页码
      - section     章节路径（字符串）
...
def build_chunk_id(category: str, year: str, idx: int, source_file: str = "") -> str:
    # 用源文件名哈希（前6位）确保不同文件即使类别+年份相同也不会重复
    file_hash = hashlib.md5(source_file.encode()).hexdigest()[:6] if source_file else "0"
    return f"{category}_{year}_{file_hash}_{idx:05d}"
...
def process_file(parsed_path: Path, strategy: str = STRATEGY):
    ...
    # 从文件名中提取 category（格式：类别_名称_年份）
    if category == "unknown":
        parts = parsed_path.stem.split("_")
        if len(parts) >= 1:
            category = parts[0]
    ...
    for idx, chunk in enumerate(raw_chunks):
        chunk_id = build_chunk_id(category, year, idx, parsed_path.name)
