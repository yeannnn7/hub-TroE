...
SYSTEM_PROMPT = """你是一个专业的日常出行规则助手，专门回答关于公共交通、铁路、民航、网约车等各类出行方式的规定和守则问题。

回答规则：
1. 只根据【参考资料】中的内容回答，不得引用或编造资料外的数据
2. 若参考资料不足以支撑回答，直接说"根据提供的资料无法回答此问题"
3. 引用具体规定时，在句末标注来源编号，如：每位旅客免费携带行李重量为20千克[1]
4. 规定条款要精确引用，不得模糊表达
5. 回答简洁，重点突出，避免无关废话"""
...
class VectorStore:
    def __init__(self, client: OpenAI):
        import faiss
        self.client    = client
        # faiss.read_index 无法处理含中文的路径，改用 Python 读取 + deserialize
        with open(INDEX_PATH, "rb") as f:
            buf = f.read()
        self.index = faiss.deserialize_index(np.frombuffer(buf, dtype="uint8"))
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "你是检索查询优化专家。将用户的问题改写为更适合从出行规则文档中检索信息的精确查询语句。"
                    "保留关键实体（出行方式、规定类别、具体条款），扩展相关关键词，不要超过50字。"
                    "直接输出改写后的查询语句，不要解释。"
                ),
            },
...
class RAGPipeline:
    ...
    def query(
        ...
        if top_score < SCORE_THRESHOLD and filter_meta is None:
            return {
                "answer": "根据出行规则知识库未能找到与该问题相关的内容，建议直接查阅原始规定文件。",
...
def main():
    parser = argparse.ArgumentParser(description="日常出行规则 RAG 问答（原生版）")
    parser.add_argument("--query",         type=str,  default=None)
    parser.add_argument("--category",      type=str,  default=None, help="出行类别，如 铁路、民航、地铁")
    parser.add_argument("--year",          type=str,  default=None, help="年份，如 2024")
