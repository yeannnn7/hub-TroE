...
def load_documents():
    ...
    if not pdf_files:
        raise FileNotFoundError(f"data/raw_pdf/ 目录下没有 PDF 文件，请将出行规则 PDF 放入该目录")
    ...
    for pdf_path in sorted(pdf_files):
        ...
        # 从文件名解析类别和年份，写入每个 Document 的 metadata
        # 新格式：类别_名称_年份，如 "铁路_国铁集团铁路旅客运输规程_2024"
        parts = pdf_path.stem.split("_")
        for doc in docs:
            doc.metadata["category"]     = parts[0] if len(parts) > 0 else ""
            # 年份取最后一个以数字开头的部分
            year = ""
...
def build_vectorstore(chunks, embeddings):
    ...
    # 保存到项目目录（save_local 内部调 faiss.write_index 无法处理中文路径，需绕过）
    import faiss as _faiss
    import numpy as _np
    import pickle
    VECTORSTORE_DIR.mkdir(parents=True, exist_ok=True)
    # 1) 保存 FAISS 索引
    buf = _faiss.serialize_index(vectorstore.index)
    with open(VECTORSTORE_DIR / "index.faiss", "wb") as f:
        f.write(_np.array(buf, dtype="uint8").tobytes())
    # 2) 保存 docstore 和 index_to_docstore_id
    with open(VECTORSTORE_DIR / "index.pkl", "wb") as f:
        pickle.dump({
