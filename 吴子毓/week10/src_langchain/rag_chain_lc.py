...
3. 引用具体规定时，在句末标注来源文件名，如：每位旅客免费携带行李重量为20千克（来源：铁路旅客运输规程）
4. 规定条款要精确引用，回答简洁，重点突出"""
...
def get_vectorstore(embeddings):
    ...
    # load_local 内部调 faiss.read_index 无法处理中文路径，手动加载
    with open(VECTORSTORE_DIR / "index.faiss", "rb") as f:
        buf = f.read()
    index = _faiss.deserialize_index(_np.frombuffer(buf, dtype="uint8"))
    with open(VECTORSTORE_DIR / "index.pkl", "rb") as f:
        data = pickle.load(f)
    vs = FAISS(embeddings.embed_query, index, data["docstore"], data["index_to_docstore_id"])
    return vs
...
def main():
    parser = argparse.ArgumentParser(description="日常出行规则 RAG 问答（LangChain LCEL 版）")
    ...
    if args.query:
        ...
        print(f"日常出行规则 RAG 问答系统（LangChain LCEL 版）")
