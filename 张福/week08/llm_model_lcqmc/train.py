...
from dataset import LCQMCDatasetForCausal, collate_fn_causal
...
def main(args):
    """主训练函数"""
    print("=" * 80)
    print("LCQMC文本匹配训练 - 基于Qwen2.5-0.5B-Instruct")
    print("=" * 80)
    ...
    # 创建数据集
    print("\n加载数据集...")
    train_dataset = LCQMCDatasetForCausal(
        str(data_dir / 'train.jsonl'),
        tokenizer,
        max_length=args.max_len
    )
    val_dataset = LCQMCDatasetForCausal(
        str(data_dir / 'validation.jsonl'),
        tokenizer,
        max_length=args.max_len
    )
    test_dataset = LCQMCDatasetForCausal(
...
def generate_report(output_dir, history, test_decoded, args):
    ...
    report_lines.append("LCQMC数据集 - Qwen文本匹配训练报告")
    report_lines.append("=" * 80)
    report_lines.append("")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='LCQMC文本匹配训练 - 基于Qwen')
