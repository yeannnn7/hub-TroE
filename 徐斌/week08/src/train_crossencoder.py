...
def main():
    ...
    train_limit = f"{args.max_train_samples:,} 条" if args.max_train_samples else "不限制"
    print(f"数据集: {dataset_name}")
    print(f"BERT 层数: {args.num_hidden_layers}  Epochs: {args.epochs}  "
          f"Batch size: {args.batch_size}  训练集上限: {train_limit}")
    ...
    # ── 训练循环 ──────────────────────────────────────────────────────────
    ckpt_path   = CKPT_DIR / f"crossencoder_{dataset_name}_best.pt"
    best_val_f1 = 0.0
    log_records = []
    ...
    # ── 训练完成，保存日志 ────────────────────────────────────────────────
    log_path = LOG_DIR / f"crossencoder_{dataset_name}_log.json"
...
def parse_args():
    ...
    parser.add_argument("--max_train_samples", default=10000, type=int,
                        help="训练集最多使用的样本数（默认 10000；0 表示不限制）")
