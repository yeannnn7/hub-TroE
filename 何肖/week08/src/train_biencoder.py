...
python train_biencoder.py --loss cosine --pool mean --num_hidden_layers 4 --epochs 3 --batch_size 32
"""
...
def parse_args():
    ...
    parser.add_argument("--num_hidden_layers", default=4,    type=int,
                        help="BERT Transformer 层数（默认 4 层快速验证；）")
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.loss == "cosine":
            train_loss = train_one_epoch_cosine(model, train_loader, optimizer, scheduler, device, epoch, args.epochs, args.margin, args.grad_accum)
        else:
            train_loss = train_one_epoch_triplet(model, train_loader, optimizer, scheduler, device, epoch, args.epochs, args.margin, args.grad_accum)
        ...
        log_records.append({
            "epoch": epoch, "train_loss": train_loss, "val_f1": val_f1, "elapsed_s": elapsed,
        })

        if val_f1 > best_val_f1:
            best_val_f1 = val_f1
            torch.save({
                "epoch": epoch, "state_dict": model.state_dict(), "val_f1": val_f1, "args": vars(args),
            }, ckpt_path)
