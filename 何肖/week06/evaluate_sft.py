...
MODEL_PATH  = resolve_qwen2_path()  # LoRA 须配 Qwen2-0.5B-Instruct
...
def load_model(model_path: str, ckpt_dir: str, device: torch.device):
    ...
    model_path = ensure_model_path(model_path, for_lora=is_lora)
    ...
    if is_lora:
        ...
        load_path = as_pretrained_path(model_path)
        tokenizer = AutoTokenizer.from_pretrained(load_path, trust_remote_code=True)
        base_model = _load_causal_lm(load_path, device)
        ...
        model = _load_causal_lm(str(ckpt_path.resolve()), device)
